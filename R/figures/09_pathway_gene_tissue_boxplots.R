...
# -----------------------------------------------------------------------------
# Script: R/figures/09_pathway_gene_tissue_boxplots.R
# Purpose: Generate publication-grade tissue-stratified boxplots for 7 DNA repair
#          pathways highlighting key genes from manuscript text/images.
# Features:
...
#   - One-way ANOVA: test tissue heterogeneity for pathway & individual genes
...
out_dir <- file.path(repo_root, "results/pathway_gene_tissue_boxplots")
...
for (pw_name in names(pathway_defs)) {
  ...
  # Compute boxplot statistics with mean as the center line
  box_stats <- sub_dt[, {
    q <- quantile(YARNNormalizedLog2, c(0.25, 0.75), na.rm = TRUE)
    iqr <- q[2] - q[1]
    m <- mean(YARNNormalizedLog2, na.rm = TRUE)
    sd_val <- sd(YARNNormalizedLog2, na.rm = TRUE)
    n_mice <- .N
    ymin <- max(min(YARNNormalizedLog2, na.rm = TRUE), q[1] - 1.5 * iqr)
    ymax <- min(max(YARNNormalizedLog2, na.rm = TRUE), q[2] + 1.5 * iqr)
  ...
  pw_sample <- sub_dt[, .(MeanExpr = mean(YARNNormalizedLog2)), by = .(Group, SampleID)]
  ...
  if (!is.null(pw_info$shapes)) {
    ...
    p <- p +
      geom_point(
        data = sub_dt_points,
        aes(x = Group, y = YARNNormalizedLog2, color = Gene),
        position = position_jitterdodge(jitter.width = 0.11, dodge.width = dodge_w, seed = 25),
  ...
  p <- p +
    labs(
      ...
      caption = paste0(
        ...
        "Dashed lines connect tissue-level mean values for each gene. ", pw_info$caption_note
      ),
      x = NULL,
      y = "YARN qsmooth log2 expression"
    ) +
...
fwrite(anova_pw_dt, file.path(tbl_dir, "01_pathway_one_way_anova_summary.csv"))
fwrite(anova_gene_dt, file.path(tbl_dir, "02_gene_one_way_anova_summary.csv"))
fwrite(summary_mean_dt, file.path(tbl_dir, "03_tissue_gene_mean_qsmooth_summary.csv"))
fwrite(used_data_dt, file.path(tbl_dir, "04_sample_qsmooth_values_used.csv.gz"))

message("All 7 pathway figures and tables successfully generated under: ", out_dir)
