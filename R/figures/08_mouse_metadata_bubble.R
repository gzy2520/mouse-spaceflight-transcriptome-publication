...
# Mouse-level experimental-design overview for the publication release.
...
fwrite(
  ...
  file.path(out, "tables/Fig_6_mouse_metadata_grouped.csv")
)
...
fig <- ggplot() +
  ...
  geom_text(
    ...
    hjust = 1, size = 3.0, colour = ink, family = "DejaVu Sans"
  ) +
  geom_text(
    data = mission_centres,
    aes(x = -1.05, y = row, label = mission_cluster),
    hjust = 1, size = 3.05, fontface = "bold", colour = ink, family = "DejaVu Sans"
  ) +
  scale_x_continuous(
  ...
  theme_minimal(base_family = "DejaVu Sans", base_size = 10.5) +
  theme(
    panel.grid = element_blank(),
    panel.background = element_rect(fill = "white", colour = NA),
    plot.background = element_rect(fill = "white", colour = NA),
    axis.text.x = element_text(colour = ink, size = 8.1, angle = 55, hjust = 1, vjust = 1, lineheight = 0.95),
    axis.text.y = element_blank(),
    axis.ticks = element_blank(),
    axis.title = element_blank(),
    plot.title = element_text(colour = ink, face = "bold", size = 16, hjust = 0, margin = margin(b = 3)),
    plot.subtitle = element_text(colour = muted, size = 9.8, hjust = 0, margin = margin(b = 7)),
    plot.caption = element_text(colour = muted, size = 8.0, hjust = 0, lineheight = 1.1, margin = margin(t = 8)),
    legend.position = "top",
    legend.box = "vertical",
    legend.box.just = "left",
    legend.title = element_text(colour = ink, face = "bold", size = 9),
    legend.text = element_text(colour = ink, size = 8.4),
    legend.key = element_blank(),
    legend.margin = margin(0, 0, 2, 0),
    plot.margin = margin(10, 14, 12, 14)
  )

png_path <- file.path(out, "Main/Fig_6_mouse_metadata_bubble.png")
pdf_path <- file.path(out, "Main/Fig_6_mouse_metadata_bubble.pdf")
ggsave(png_path, fig, width = 24, height = 15, units = "in", dpi = 450, bg = "white", limitsize = FALSE)
ggsave(pdf_path, fig, width = 24, height = 15, units = "in", device = grDevices::cairo_pdf, bg = "white", limitsize = FALSE)

message("MOUSE_METADATA_FIGURE_PASS: ", png_path, " and ", pdf_path)
