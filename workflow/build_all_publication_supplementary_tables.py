#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Portable publication table export from frozen inputs and figure provenance.

Requires Python 3, pandas and openpyxl. Retains the approved exporter's cell
selection and formatting. S4 is an unchanged supplied software workbook, not a
recalculated statistical model. Output must be a new or empty directory.
"""
...
CSV_DIR = None
FIGURE_DIR = None
EXTERNAL_FILE = Path(REPO_ROOT) / "data/external_statistics/2way_anova_7_pathways.xlsx"
...
def build_tables():
    ...
    df_s1_cohorts_raw = pd.read_csv(os.path.join(FIGURE_DIR, "provenance/Fig_1b_mouse_metadata_grouped.csv"))
    ...
    df_s1_samples_raw = pd.read_csv(os.path.join(FIGURE_DIR, "provenance/Fig_6_mouse_sample_metadata_complete_audit.csv"))
    ...
    s1_path = os.path.join(OUTPUT_DIR, "Table_S1_Cohort_and_Sample_Metadata.xlsx")
    ...
    s2_path = os.path.join(OUTPUT_DIR, "Table_S2_DDR_Overlap_and_Common_DEGs.xlsx")
    ...
    df_s3_sizes_raw = pd.read_csv(os.path.join(REPO_ROOT, "results/tables/08_Table_S3_pathway_repertoire_sizes.csv"))
    df_s3_sizes = df_s3_sizes_raw[['Pathway', 'MemberGeneCount']].rename(columns={'MemberGeneCount': 'Member_Gene_Count'})

    df_s3_tissue_raw = pd.read_csv(os.path.join(REPO_ROOT, "results/tables/09_Table_S3_per_tissue_sample_summary.csv"))
    ...
    df_s3_samples_raw = pd.read_csv(os.path.join(REPO_ROOT, "results/tables/01_seven_pathway_member_gene_counts_per_source_sample_wide.csv"))
    ...
    s3_path = os.path.join(OUTPUT_DIR, "Table_S3_Seven_Pathways_Gene_Repertoires_and_Detection.xlsx")
    write_excel_sheets(s3_path, s3_sheets)
    save_csvs("Table_S3", s3_sheets)

    print("\n=== Step 4: Copying supplied two-way ANOVA workbook (no recalculation) ===")
    metadata = json.loads((EXTERNAL_FILE.parent / "provenance.json").read_text())
    if hashlib.sha256(EXTERNAL_FILE.read_bytes()).hexdigest() != metadata["sha256"]:
        raise ValueError("External statistics workbook differs from supplied input SHA-256")
    shutil.copyfile(EXTERNAL_FILE, Path(OUTPUT_DIR) / EXTERNAL_FILE.name)
    ...
    s5_path = os.path.join(OUTPUT_DIR, "Table_S5_Functional_Pathway_and_Hallmark_GSEA.xlsx")
    ...
    s6_path = os.path.join(OUTPUT_DIR, "Table_S6_Pathway_Association_and_FisherZ_Integration.xlsx")
    ...
    s7_path = os.path.join(OUTPUT_DIR, "Table_S7_DSB_Repair_Meta_Analysis_Matrix.xlsx")
    ...
    df_s8_raw = pd.read_csv(os.path.join(REPO_ROOT, "results/tables/01_SSB_tissue_meta_log2FC_pvalue_matrix_without_Neil2.csv"))
    ...
    s8_path = os.path.join(OUTPUT_DIR, "Table_S8_SSB_Repair_Meta_Analysis_Matrix.xlsx")
    ...
    manifest_rows = [
        ...
        {"Table_ID": "Table S4", "Title": "Supplied Two-Way ANOVA Software Results", "Teacher_Citation": "External significance input supplied by user; model not recalculated", "Sheets": "NER, MMR, HR, FA, BER, A-EJ, NHEJ (unchanged software workbook)", "File": EXTERNAL_FILE.name},
    ...
    manifest_path = os.path.join(OUTPUT_DIR, "Table_S_Manifest.csv")
    df_manifest.to_csv(manifest_path, index=False)
    print(f"Manifest written: {manifest_path}")


def main(argv=None):
    global OUTPUT_DIR, CSV_DIR, FIGURE_DIR
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--output-dir", required=True, type=Path,
                        help="New or empty supplementary table directory")
    parser.add_argument("--figure-dir", required=True, type=Path,
                        help="Generated figure directory containing provenance/*.csv")
    args = parser.parse_args(argv)
    OUTPUT_DIR = args.output_dir.expanduser().resolve()
    FIGURE_DIR = args.figure_dir.expanduser().resolve()
    CSV_DIR = OUTPUT_DIR / "csv"
    if OUTPUT_DIR.exists() and (not OUTPUT_DIR.is_dir() or any(OUTPUT_DIR.iterdir())):
        parser.error("--output-dir must be new or empty; no existing files are removed")
    build_tables()
