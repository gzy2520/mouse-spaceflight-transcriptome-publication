...
if (!length(script_arg)) stop("Cannot locate workflow/run_pipeline.R", call. = FALSE)
script_path <- normalizePath(sub("^--file=", "", script_arg[[1L]]), mustWork = TRUE)
root <- normalizePath(file.path(dirname(script_path), ".."), mustWork = TRUE)
args <- commandArgs(trailingOnly = TRUE)
out <- if (length(args)) args[[1L]] else file.path(root, "reproduced_results")
if (!grepl("^/", out)) out <- file.path(root, out)
if (file.exists(out)) {
  stop("Output already exists; choose a new directory: ", out, call. = FALSE)
}
...
# The published tables are already the final analysis products. Copying them
...
status <- system2("Rscript", c("tests/validate_publication_contract.R", shQuote(out)))
if (!identical(status, 0L)) stop("Publication validation failed", call. = FALSE)
message("Publication outputs reproduced in: ", out)
